...
class SupabaseDocument implements DocumentAdapter {
  ...
  async set(data: any, options?: { merge?: boolean }): Promise<void> {
    // merge:true must UPDATE existing columns only — a full upsert of a partial
    // payload can fail RLS/insert constraints and is the root cause of several
    // "Failed to save" errors (link-in-bio profile, theme settings, etc.).
    if (options?.merge) {
      let query = this.supabase
        .from(this.tableName)
        .update(data)
      ...
      // Row missing — fall through to insert via upsert
      const payload = this.businessId
        ? { id: this.id, ...data, businessId: this.businessId }
        : { id: this.id, ...data };
