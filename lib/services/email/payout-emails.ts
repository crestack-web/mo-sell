...
/**
 * Notify the merchant that the bank transfer succeeded and funds have been
...
export async function sendPayoutCompletedEmail(data: PayoutCompletedEmailData) {
  ...
  const html = renderShell({
    ...
    subtitle: `Your ${amount} payout has been paid into your bank account.`,
    body: `
      <p>Great news — the transfer for your payout from <strong>${storeName}</strong> has been completed. The money should now be in your bank account (or land within a few hours depending on your bank).</p>
      <div style="background: #F7FAFC; border: 1px solid #E0EFFA; border-radius: 12px; padding: 20px; margin: 16px 0;">
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
          <tr>
      ...
      <p>You can confirm this in your payout history — the status is now <strong>Completed</strong>.</p>
      <p>If the funds don't appear within 24 hours, contact your bank with the reference above, or reply to this email and we'll help.</p>
    `,
    ctaText: 'View payout history',
    ctaUrl: `${APP_URL()}/dashboard/earnings`,
  });
  ...
  return sendEmail({
    to: data.email,
    name,
    subject: `Your ${amount} payout has been paid to your bank ✅`,
    html,
  });
}
