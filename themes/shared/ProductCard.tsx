...
export function ProductCardWithTheme({ themeId, product, storeSlug, currency }: ThemeProductCardProps & { themeId: string }) {
  ...
  // ── Atelier — editorial, no card chrome, hairline rule, quiet "+ Add" link ──
  if (themeId === 'atelier') {
    return (
  ...
  // ── Citrus — round thumbnail, centered, overlapping round add button ──
  if (themeId === 'citrus') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
  ...
  // ── Nordly — horizontal row, thumbnail left, text right, hairline border ──
  if (themeId === 'nordly') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
        style={{ textDecoration: 'none', color: 'inherit', display: 'flex', alignItems: 'center', gap: 12, padding: 10, background: t.surface, border: `1px solid ${t.border}`, borderRadius: t.radius }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      >
  ...
  // ── Neo Tech — spec card, NEW badge, rating row, corner add button ──
  if (themeId === 'neotech') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
  ...
  // ── Terra — "Handmade" tag, full-width Add button beneath ──
  if (themeId === 'terra') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
  ...
  // ── Volt — hard edges, NEW badge, boxed price chip, square add button ──
  if (themeId === 'volt') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
  ...
  // ── Botanica — soft rounded card, price pill floating on image ──
  if (themeId === 'botanica') {
    return (
      <Link href={`/store/${storeSlug}/product/${product.id}`}
  ...
  // ── Prism — glass card, floating circular thumbnail, glowing pill add button ──
  return (
    <Link href={`/store/${storeSlug}/product/${product.id}`}
