...
export function AbbyProductPage({ product, storeSlug, currency }: ThemeProductPageProps) {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 20px 80px' }}>
      <nav style={{
        ...
        <Link href={`/store/${storeSlug}`} style={{ color: 'var(--sf-blue)', textDecoration: 'none' }}>Store</Link>
        <span>/</span>
          ...
          {product.images.map((img, i) => (
            <button key={i} onClick={() => setActiveImg(i)}
              style={{
                ...
                border: i === activeImg ? '2px solid var(--sf-blue)' : '2px solid var(--sf-border)',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {product.category && (
          <span style={{
            ...
            letterSpacing: '0.08em', color: 'var(--sf-blue)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--sf-blue)' }}>
          ...
          {discount && (
            <span style={{
              background: '#5383FF', color: '#fff', fontSize: '0.72rem', fontWeight: 800,
        ...
        {product.productType === 'digital' && (
          <div style={{
            padding: '10px 14px', background: 'var(--sf-blue-lt)',
            border: '1px solid var(--sf-border)', borderRadius: 'var(--sf-radius-sm)',
            fontSize: '0.82rem', color: 'var(--sf-blue)', fontWeight: 700,
          }}>
        ...
        {!isOutOfStock && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'stretch', marginTop: 4 }}>
            <div style={{
              ...
              <button onClick={() => setQty(Math.max(1, qty - 1))}
                style={{
                  ...
                  fontSize: '1.1rem', fontWeight: 800, color: 'var(--sf-blue)', cursor: 'pointer',
                }}>−</button>
              <span style={{
            ...
            <button onClick={handleBuy}
              ...
              style={{
                flex: 1, height: 46,
                background: buying ? '#10B981' : 'var(--sf-blue)',
