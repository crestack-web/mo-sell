...
/**
 * The 5 recipe pages rendered as plain <Page> elements (no <Document>),
 * so they can be composed alongside other pages in a single PDF.
 */
export const RecipeEbookPages = ({ data, images }: EbookRecipe5PageProps): ReactElement[] =>
  data.pages.map((page, i) => {
    ...
    if (page.type === 'cover') {
      ...
      return (
        <Page key={`recipe-${i}`} style={[styles.page, { padding: 0 }]}>
    ...
    if (page.type === 'recipe') {
      ...
      return (
        <Page key={`recipe-${i}`} style={styles.page}>
    ...
    // cta page
    return (
      <Page key={`recipe-${i}`} style={[styles.page, { justifyContent: 'center' }]}>
...
export const EbookRecipe5Page = ({ data, images }: EbookRecipe5PageProps) => (
  <Document title={data.title}>{RecipeEbookPages({ data, images })}</Document>
);
