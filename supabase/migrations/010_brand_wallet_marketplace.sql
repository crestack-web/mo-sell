...
-- Unquoted mixed-case identifiers would be folded to lowercase by Postgres and
-- break the adapter's payload passthrough.
...
alter table public.brands add column if not exists "topupCurrency" text;
...
drop policy if exists purchased_videos_select_own on public.purchased_videos;
create policy purchased_videos_select_own
  on public.purchased_videos
  for select
  to authenticated
  using ("brandId" in (select id::text from public.brands where "userId" = auth.uid()));

drop policy if exists purchased_videos_insert_own on public.purchased_videos;
create policy purchased_videos_insert_own
  on public.purchased_videos
  for insert
  to authenticated
  with check ("brandId" in (select id::text from public.brands where "userId" = auth.uid()));

drop policy if exists purchased_videos_update_own on public.purchased_videos;
...
drop policy if exists purchased_videos_delete_own on public.purchased_videos;
